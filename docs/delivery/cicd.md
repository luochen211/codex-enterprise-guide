...
如果每次发布都靠人记得“先测试、再构建、再上传”，迟早会漏一步。CI/CD 把这些动作写成固定流程，让每次改动都留下检查和发布记录。

CI 负责检查改动能不能进入下一步；CD 负责把通过检查的版本交付或部署出去。
...
有了 CI/CD，每次改动走同一套检查。成功版本从哪里来、失败停在哪一步、线上应该回到哪个版本，都有记录可查。
...
先复现，再修复。直接登录服务器改线上文件，只会制造一个 Git 里找不到的新版本。
