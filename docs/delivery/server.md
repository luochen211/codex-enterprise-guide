...
本地能打开，不等于别人能访问。部署要解决的是：代码放到哪里运行、用户从什么地址进入、出问题以后怎样找到版本并恢复。

这个文档站只需要 GitHub Pages。等项目出现后端、数据库、长期进程或更复杂的权限，再考虑应用托管、云服务器或容器。
...
```text
https://luochen211.github.io/codex-enterprise-guide/
```
